...
export function PreviewStep(props: Props) {
  ...
  useSubStep(page ? t("editor.title", { n: page.spread }) : null, () => setSelected(null));
    ...
    <>
      ...
      {page ? (
        ...
        <>
          ...
          <BookFlipbook book={props.book} onEditPage={editPage} reserveRem={38} />
          <StepFooter>
...
function PageEditor({ page, rewritesLeft, imageEditsLeft, editorial, rewriteCharacter, onClose }: Props & { page: PreviewPage; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <StepFooter>
        <Button variant="next" className="w-full sm:w-auto" onClick={onClose}>
          {t("common.done")}
        </Button>
      </StepFooter>
    </>
