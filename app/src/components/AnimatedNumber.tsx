...
export function AnimatedNumber({
  ...
  // Pri obmedzení animácií sa mení len trvanie (rovnaké HTML na serveri aj v prehliadači).
        ...
        {chars.map((char, i) => {
          ...
          return (
            <motion.span
              ...
              exit={{ opacity: 0, y: "0.35em", filter: "blur(2px)", transition: spring === SPRING ? { duration: 0.15 } : INSTANT }}
              transition={spring}
              className="inline-block"
            >
              {isDigit ? <DigitColumn digit={Number(char)} transition={spring} /> : char}
            </motion.span>
          );
        })}
      </AnimatePresence>
